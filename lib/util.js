...
module.exports = {
    ...
    canIreact: function(channel) {
        return channel.permissionsFor(channel.guild.me).has("ADD_REACTIONS");
    },
        ...
        session._inactive = setTimeout(() => {
            if (session && session.connection) {
                ...
                if (!session.current || (session._empty && (memberCount === 1) && (memberFirst.id === channel.client.user.id))) {
                    this.runCommand("leave", [null, null, session._channel]);
                } else {
    ...
    checkUserVoiceChannel: function(message, session) {
        ...
        if (session && session.connection) {
            if (!channel || (message.guild.me.voice.channel.id !== channel.id)) {
    ...
    fadeOutAndNext: function(message, session) {
        ...
        session._timeout = setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                this.runCommand("next", [message, null, true]);
    ...
    isSCMR: function(id) {
        return (id === "743152717995704320");
    },

    isSnesBot: function(id) {
        return (id === "689821660358246401");
    },
    ...
    handleError: function(error) {
        ...
        if (this && this.reply) {
            this.reply({
                content: "An unexpected error occurred. (error ID: " + hash + ")",
                allowedMentions: {
                    repliedUser: false
                }
            });
